...
:class:`~probot_drivers.machine_smu_probot.SMUKeysightProbotMachine`.

The host class must provide:

* ``self.smu`` - the raw PyVISA resource (supplied by
  :class:`~probot_drivers.smu_keysight_probot.SMUKeysightProbot`),
* ``self.pico_instrument`` - the light driver
  (:class:`~probot_drivers.pico_probot.PicoProbot`),
    ...
    Returns:
        Dict[str, Any]: result envelope with keys:
            ...
            - ``outputs`` (list[dict]): one record per saved file, each
              ...
              collected by :meth:`MeasurementProbot.savefile` /
              :meth:`MeasurementProbot.savefile_1`.
...
class MeasurementProbot:
    """Mixin providing the probot Keysight SMU measurement routines.

    Mixed into :class:`~probot_drivers.machine_smu_probot.SMUKeysightProbotMachine`,
