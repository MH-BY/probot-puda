...
---------------------------------
SCPI helpers, data-persistence, plotting, and parameter-loading methods are all
private (``_``-prefixed) and not reflected as PUDA primitives.
"""
...
_DEFAULT_PARAM_DIR = os.path.join(
...
def _measurement_result(fn):
    ...
    Resets the per-run output accumulator, runs the measurement body, and
    packages a JSON-serialisable result dict.
    ...
    Returns:
        Dict[str, Any]: result envelope with keys:
            ...
            - ``result`` (Any): whatever the underlying routine returned.
    """
    @functools.wraps(fn)
...
class SMUKeysightDriver:
    ...
    def __init__(
        ...
        Args:
            ...
            param_dir: writable directory for measurement parameter CSVs.  Defaults to
                the ``Parameters/`` folder next to this file, then
                ``PROBOT_PARAM_DIR`` env var.
            data_dir: root directory for output data.  Defaults to
                ``./Data/Keysight``, then ``PROBOT_DATA_DIR`` env var.
        """
        # VISA transport state
        self._smu_address = smu_address
        self._smu_device_no = smu_device_no
        self._rm = None
        self.smu = None           # raw pyvisa resource; consumed by measurement routines

        # Pico light (also aliased as pico_instrument for the verbatim measurement bodies)
        self._light = PicoProbot(ip=pico_ip, device_id=pico_id)
        self.pico_instrument = self._light  # alias used by verbatim measurement routines
        ...
        logger.info(
            "SMUKeysightDriver initialised (smu_address=%s, pico_ip=%s, param_dir=%s)",
    ...
    * **Analysis** - :meth:`Pot_Dep_Calculation` (and ``Keysight_HT_PotDep``)
    ...
    def _send_pulse_train_to_keysight(self, compliance, pulse_train_string,trigger_count,trigger_period,measure_delay):
        ...
        try:
            ...
            self.smu.write(f":TRIG:ACQ:DEL {measure_delay}") # Set the measure delay. Preferably, the measurement is performed at the centre of the trigger.
                ...
                # ********** RUN MEASUREMENT ********** 
            logger.debug('INNITIATE MEASURREMENT')
            ...
            logger.debug("%s", self.smu.query("SYST:ERR?")) # Check error buffer. If there was any error in the execution of SCPI commands. 
    ...
    def _string_to_dataframe(self,output_string):# change the output data (string) to dataframe
        ...
        try:
            ...
            logger.debug('Change output_string to output_table')
    ...
    def _savefile(self,output_table,df_parameters,cell_number,keyword):
        ...
        try:
            ...
            logger.info("File saved as: " + file_name)
    ...
    def _savefile_1(self,output_table,df_parameters,cell_number,keyword,plot_type):
        ...
        try:
            ...
            logger.info(f"File saved as: {base_path}.csv")
    ...
    def _make_graph(self,output_table,cell_number):
        ...
        try:
            logger.debug('start making graph')
            ...
            plt.close('all')
            logger.debug('finish making graph')
        except:
            logger.debug('No graph to plot')
            pass   
    ...
    def _make_graph_IV_1(self,output_table,cell_number,plot_type,save_path=None):
        ...
        try:
            ...
            if save_path:
                ...
                logger.info(f"Plot saved as: {save_path}.png")
                plt.close(fig) 
                
            plt.show()
        except Exception as e:
            logger.error(f"No graph to plot: {e}")
            pass
    ...
    def Keysight_Digital_Sweep(self, cell_number: int) -> Dict[str, Any]:
        ...
        for i in range(int(no_cycles)):
            logger.info('Measuring sweep cycle: '+str(i+1))
    ...
    def Keysight_Analog_Sweep(self, cell_number: int) -> Dict[str, Any]:
        ...
        for i in range(int(no_cycles)):
            logger.info('Measuring set sweep cycle: '+str(i+1))
        ...
        for i in range(int(no_cycles)):
            logger.info('Measuring reset sweep cycle: '+str(i+1))
    ...
    def Keysight_set_reset_sweep(self, cell_number: int) -> Dict[str, Any]:
        ...
        def run_sweep (sweep, label, cycle_idx):
            ...
            try:
                ...
                logger.error(f"Error during {label} sweep: {e}")
                return pd.DataFrame()
        ...
        if mode == "loop":
            for i in range(int(no_cycles)):
                logger.info(f"Measuring SET sweep cycle: {i+1}")
                df_sweep_total = pd.concat([df_sweep_total, run_sweep(set_sweep, "Set", i)], ignore_index=True)
    
                logger.info(f"Measuring RESET sweep cycle: {i+1}")
    ...
    def Keysight_Substrate_R(self, cell_number: int) -> Dict[str, Any]:
        ...
        for i in range(int(no_cycles)):
            logger.info('Measuring cycle: '+str(i+1))
    ...
    def Keysight_JV_PV(self, cell_number: int) -> Dict[str, Any]:
        ...
        for i in range(int(no_cycles)):
            ...
            #---FORWARD----
            logger.info('Measuring cycle '+str(i+1)+' forward')
            ...
            #---REVERSE
            logger.info('Measuring cycle '+str(i+1)+' reverse')
    ...
    def Keysight_Voltage_list(self, cell_number: int) -> Dict[str, Any]:
            ...
            logger.debug("%s", pulse_train)
            logger.debug("%s", pulse_train.shape)
            logger.debug("%s", type(pulse_train[0]))
    ...
    def _Pot_Dep_Calculation(self,df, cell_number, keyword):
        ...
        try:
            ...
            logger.info('no file to save at pot_dep_calculation')
            pass        
    ...
    def Keysight_Potent_Depress_2(self, cell_number: int) -> Dict[str, Any]:
        ...
        #change the output_string to output_table (DF)
        output_table = self._string_to_dataframe(output_string)
        logger.debug("output_table from SMU acquired")
        ...
        #make a measurement overall graph
        try:
            logger.debug('start making overall graph')
            fig, ax1 = plt.subplots(figsize=(10,2)) # Plot current and voltage vs time.
            ax2 = ax1.twinx() # Plot double-y axis.
            ax1.plot(output_table["Time (s)"],output_table["Current (A)"], color='b', alpha=0.5) # Plot current vs time.
            ax2.plot(output_table["Time (s)"], output_table["Voltage (V)"], color='r', alpha=0.5) # Plot voltage vs time.
            ...
            plt.show(block = False)
            plt.close('all')
            logger.debug('finish making overall graph')
        except:
            logger.debug('No graph to plot')
            pass 
        
        try:
            logger.debug('start making read graph')
            ...
            plt.show(block = False)
            plt.close('all')
            logger.debug('finish making read graph')
        except:
            logger.debug('No graph to plot')
            pass 
    ...
    def Keysight_HT_PotDep(self, cell_number: int) -> Dict[str, Any]:
        ...
        logger.debug("--Start innitial sampling--")
        ...
        logger.debug("--Innitial sampling finished--")
        
        logger.debug('do analog sweeping')
        ...
        logger.debug("--Start BO sampling. Total campaign = "+str(campaign))
    ...
    def Keysight_Voc_decay_ON_OFF_Variation(self, cell_number: int) -> Dict[str, Any]:
        ...
        #send to instrument
        if self.smu is None:
            logger.info("Error: SMU is not connected. Aborting measurement.")
            return None
        ...
        try:
            ...
            logger.info(f"Error communicating with SMU: {e}")
            output_string = 'NONE'
...
# The measurement list advertises ``Keysight_Digital_Retention`` while the
# implementation is ``Keysight_Digital_Endurance`` (reads
# ``parameter_Keysight_Digital_Retention.csv``).  Alias so PUDA + GUI resolve both.
