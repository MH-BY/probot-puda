...
|---|---|---|---|
| `smu-keysight-probot/` | `smu-keysight-probot` | Keysight SMU + Pico light | `SMUKeysightProbotMachine` |
| `stage-probot/` | `stage-probot` | Ender 3-axis stage | `StageProbot` |
...
```
probot-puda/
├── pyproject.toml              # uv workspace: members = smu-keysight-probot, stage-probot, gui, probot_drivers
...
│       ├── smu_keysight_probot.py  # SMUKeysightProbot  — raw PyVISA session (transport only)
│       ├── pico_probot.py          # PicoProbot         — Pico G2V light
│       ├── stage_probot.py         # StageProbot        — Ender stage (also the stage edge's machine)
│       ├── measurement_probot.py   # MeasurementProbot  — the ~22 Keysight_* routines (VERBATIM port)
│       ├── machine_smu_probot.py   # SMUKeysightProbotMachine — composes SMU+light+measurements
│       ├── orchestrator_probot.py  # run_scan() — the shared cell-scan loop
...
├── smu-keysight-probot/        # edge 1 (main.py + ViPSA scaffold + working Parameters/)
├── stage-probot/               # edge 2 (main.py + ViPSA scaffold)
...
2. **A full cell scan spans both edges → PUDA orchestrates it.** No single edge
   can run a whole scan. `orchestrator_probot.run_scan()` is the **canonical
   ...
   `stage-probot` move/probe primitives interleaved with `smu-keysight-probot`
   measurement primitives.

3. **Measurement bodies are ported VERBATIM** from `../probot-source/keysight.py`.
   `measurement_probot.py` was generated by transforming that file: only the
...
8. **Measurements return a uniform `Dict[str, Any]` envelope** via the
   `_measurement_result` decorator (in `measurement_probot.py`) — `{measurement,
...
- **GUI shims keep the GUI's required names** (`keysight.py`/`KeysightInstrument`,
  ...
  the original, edited only to call `orchestrator_probot.run_scan` from its two
...
- **`Keysight_Digital_Retention`** is advertised by `measurement_list()` but the
  ...
  `machine_smu_probot.py` makes the advertised name resolve. (Pre-existing source
  quirk.)
